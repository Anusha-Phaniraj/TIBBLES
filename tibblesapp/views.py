...
def home(request):
    if request.user.is_authenticated:
        if request.user.username == '11':
            ...
            cursor.execute("SELECT DISTINCT f.faculty_id, f.faculty_name FROM faculty F NATURAL JOIN pref_sub p NATURAL JOIN pref_slot s WHERE f.d_no = 1")
            fp = cursor.fetchall()
    ...
    if request.method=="POST":
        ...
        if password==cp:
            try:
                ...
                user.save()
                cursor.execute("insert into faculty values ('{}', '{}', '{}', '{}')".format(int(id), name, int(dept), int(prio)))
                connection.commit()
            except:
...
def slot_pref(request):
    ...
    if request.user.is_authenticated:
        if request.method=="POST":
            m=sql.connect(host="localhost", user="root", passwd="o!rul8,2?", database='tt')
            cursor=m.cursor()
            k=request.POST
            iq=request.user
            day = 'MONDAY'
...
def display(request):
    ...
    try:
        ...
        if len(sem3_tt) == 0:
            ...
            cursor.callproc('tt3')
            cursor.execute("SELECT * FROM timetable3 ORDER BY   \
                            CASE    \
        ...
        if len(sem5_tt) == 0:
            ...
            cursor.callproc('tt5')
            cursor.execute("SELECT * FROM timetable5 ORDER BY   \
                            CASE    \
                            WHEN day = 'Monday' THEN 1  \
                            WHEN day = 'Tuesday' THEN 2  \
                            WHEN day = 'Wednesday' THEN 3 \
        ...
        cursor.execute("SELECT s.subject_name, s.subject_code, f.faculty_name FROM subject_allocation a NATURAL JOIN faculty f NATURAL JOIN subject s WHERE subject_code like '%5_' ORDER BY substring(s.subject_code, 5), s.subject_code ")
